...
그 후 `python manage.py makemigraions`으로 migration을 만들고 `python manage.py migrage`로 db를 업데이트 해준다.
...
하지만 admin 페이지에 들어가서 User를 확인해보면 에러가 발생하는데, first_name과 last_name을 editable=False로 지정해서 발생하는 문제다.
...
admin 페이지로 돌아와보면 first_name과 last_name이 보이는데, 우리가 사용하지 않는 필드이므로 없애고 싶다.
