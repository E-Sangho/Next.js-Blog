...
앞서 말했듯 REST는 유지보수를 위한 원칙으로, 위 내용은 모두 유지보수와 관련있다.
여기서 문제가 되는 것은 Uniform Interface인데, 일단은 나머지를 간단히 설명하겠다.
우선 클라이언트 서버 구조를 보자.
1980년도 전의 컴퓨터는 몇몇 사람을 위한 장비일뿐 널리 쓰이는 장비가 아니었고, 그 시스템을 분산할 필요가 없었다.
그러다가 인터넷이 생기면서 컴퓨터가 여러 장소에 분산되게 된다.
이로 인해 자연스럽게 자원을 관리하는 서버와, 사용자에게 보여주는 클라이언트 개념이 생겼다.

둘이 나뉘면서 생긴 가장 큰 장점은 서로 독립적으로 코드를 만들 수 있다는 점이다.
클라이언트는 최대한 여러 상황에서 널리 쓰이도록 발전했다.
웹 페이지는 크롬, 사파리, 파이어폭스 어떤 브라우저를 사용해도 정상적으로 동작한다.
뿐만 아니라 PC, 스마트폰, 태블릿 같이 장비가 달라도 같은 페이지를 보여준다.
이처럼 여러 환경에서 쉽게 쓰이는 것을 이식성(portability)라고 한다.
서버는 새로운 기능을 추가하기 쉽게 발전했는데, 이를 확장성(scalability)라고 한다.
이들이 당연하게 보일지 모르겠지만 그렇지 않다.
휴대폰 애플리케이션은 os마다 다른 스토어를 사용하고, 각자 따로 만들어줘야 한다.
같은 형태를 보여줌에도 다른 코드를 작성해야 하기 때문이 이식성이 나쁘다.
또한 애플리케이션은 새로운 기능을 추가하면 업데이트 해야만 작동하고, 이는 확장성이 별로인 것이다.

stateless는 요청의 정보를 저장하지 않는다는 것이다.
이로 인해 같은 사용자가 요청하더라도 다른 사람이 요청한 것처럼 작동한다.
여기서 정보를 저장하지 않는 다는 것이 중요한데, 이전의 정보 없이도 정상적으로 작동한다는 것이다.
이는 곧 각 요청마다 이해하는데 필요한 모든 정보가 들어 있어서 별도로 정보가 필요 없다는 뜻이다.
동시에 클라이언트는 서버측에 저장된 정보를 사용하지 못하고, 클라이언트는 필요한 정보를 스스로 보관해야 한다.
그래서 stateless로 만들면 필요한 정보가 모두 들어 있어서 요청을 이해하기 쉽다.
또한 에러가 발생해도 요청을 다시 보내는 것으로 간단하게 해결할 수 있다.
만약 추가적인 정보가 필요했다면 이를 다시 저장하는 과정을 거쳐야 하므로 불편했을 것이다.
마지막으로 이 요청만 신경쓰면 되므로 서버에서 신경쓸 것이 별로 없고 기능을 확장하기도 편하다.

stateless의 도입으로 요청을 독립적으로 만든 것은 좋지만, 문제는 같은 요청이 반복때 낭비가 심하다는 것이다.
...
이 기능을 추가하면 브라우저에서 일을 처리할 수 있으므로, 사전에 모든 코드를 구현할 필요가 없다.
예를 들어 SSR은 서버에서 모든 코드를 구현한 다음 페이지를 전달한다.
그런데 CSR은 클라이언트 측에서 자바스크립트를 실행시켜 페이지를 만들게 한다.
이로 인해 서버의 부하를 줄이고, 클라이언트는 확장성이 늘어난다.
...
HTML은 이 모든 기능을 만족한다.
HTML을 보면 태그를 해석해서 페이지를 만들 수 있다.
태그를 해석하기 위해 HTML 외에 어떤 정보도 필요하다.
또한 HTML의 Link 태그로 하이퍼링크 기능도 지원하므로 간단하게 만족한다.
그러므로 HTML을 사용하는 웹 역시 REST의 조건을 모두 만족한다.

하지만 API는

REST의 핵심은 서로 독립적으로 개발 가능하고, 한쪽이 변해도 다른쪽을 수정할 필요가 없다는 것이다.
웹 페이지가 변경 되어도 브라우저를 수정할 필요가 없고, 반대로 브라우저가 업데이트 되어도 페이지를 변경할 필요가 없다.
또한 HTTP나 HTML 명세가 변경되어도 웹에 아무런 문제가 없다.
이를 당연하게 여길지도 모르지만 모바일 애플리케이션을 생각하면 당연하지 않다.
서버가 변경되면 그에 맞춰서 애플리케이션을 업데이트 해야하고, 버전이 맞지 않으면 아예 작동되지 않는 경우가 많다.
...
하지만 REST 구조는 훌륭하므로 그 의미를 약간만 틀어서 아래처럼 REST API를 정의한다.
...
결론 REST API는 URI에 자원을 지정하고 HTTP Method로 그 자원을 관리한다.
