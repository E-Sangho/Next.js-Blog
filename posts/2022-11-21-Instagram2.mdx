...
Roy Fielding이란 사람이 HTTP 작업에 참여하게 되면서 고민이 생겼다.
HTTP를 고치면 기존에 사용하던 웹과 호환성 문제가 발생하는데, 어떻게 웹은 망가뜨리지 않고 만들 수 있을지에 관한 것이었다.
이 생각은 이후에 정리되어 박사 논문으로 발표되는데 이것이 바로 REST다.

REST에 관한 설명을 이어가기 전에 잠시 API(Application Programming Interface)를 설명하려 한다.
애플리케이션에서 운영체제, 프로그래밍 언어, 다른 애플리케이션의 기능을 가져다 쓰고 싶다고 하자.
애플리케이션 제작자가 모든 기능을 알아서 기능을 수행할 수 있다면 좋겠지만 이는 현실적으로 불가능하다.
운영체제가 세부적으로 어떻게 작동하는지, 다른 사람이 작성한 애플리케이션 기능을 모두 알수는 없다.
그래서 이들은 애플리케이션에서 가져다 쓰기 쉽게 기능을 만들어둔다.
예를 들어서 print를 생각해보자.
print를 사용하면 원하는 내용을 간결하게 출력할 수 있지만, 그 뒤에 운영체제와 프로그래밍 언어가 어떻게 작동하는지까지 알 필요는 없다.
이 정보는 너무 과도하며 단지 사용자는 print라는 명령어를 입력하면 내용이 출력된다는 사실만 알면 된다.
API는 요청에 따라 응답을 보내주는데 이를 사용자가 쉽게 사용하기 위해 만들어졌다.
...
이중 몇몇 상품은 통신 기술이 들어 있어서 다른 사람과 소통하는데 쓸 수 있다.
그런데 상품과 통신 기술은 전혀 다른 것이다.
일부 상품에 통신 기술이 들어 있긴 하지만 포함관계도 아닌 것이다.

REST(Representational State Transfer)는 쉽게 말하면 '네트워크에서 통신을 구성할 때 이런 구조로 설계하라는 지침' 정도로 볼 수 있다.
본디 네트워크 통신을 위해 제시하였으나, 현실적으로 온라인 '네트워크'의 지분 중 태반을 차지하는게 월드 와이드 웹이기 때문에 '웹' 기반의 전송을 위해 쓰이는 경우가 대부분이다.
태생 자체가 데이터 송수신에 최적화 되어 있다보니 이를 위한 웹 API 쪽에서 굉장히 많이 쓰인다. 이를 'REST API'라고 부르는데, 이제는 그냥 '웹 API'와 동일하다고 볼 수 있을 정도로 보편화되었다.
...
Q.어떻게 인터넷에서 정보를 공유할 것인가? WEB(1991)
A. 하이퍼 텍스트로 연결한다.
...
Roy T. Fielding
HTTP를 고치면 기존의 웹이랑 호환성 문제 발생 -> 어떻게 하면 웹을 망가뜨리지 않고 개선할 수 있을까? -> HTTP Object Model
1998에 REST 공개
XML-RPC -> SOAP
Salesforce API(SOAP로 제작). 너무 복잡함
flickr API SOAP와 REST 제작. 훨씬 짧고 편함
-> REST 승리

여러 회사에서 REST를 개발했지만 Roy는 그런건 REST가 아니라고 남김.
REST를 개발한 사람과, 개발자들 사이엔 REST가 무엇인지에 관해 갭이 존재

REST = 아키텍쳐 스타일(= 제약조건의 집합)
